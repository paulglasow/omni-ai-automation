 ...
 *
 * Multi-model AI chat interface optimized for a 2026 GIS + affordable
 * housing developer workflow. Routes queries to the right AI automatically
 * via the "Assist" smart-stack mode.
 *
 * SETUP:
 ...
 * @property {string|null} routedTo   - Provider chosen by Assist mode (e.g. 'perplexity')
 * @property {Date} timestamp
 */

// ── Constants ─────────────────────────────────────────────────────────────────
const AI_MODELS = [
  { id: 'assist',     label: 'Assist (Smart Stack)', emoji: '🧭', color: '#7c3aed' },
...
// Friendly labels shown when Assist mode reveals which provider it chose
const ROUTED_TO_LABELS = {
...
export default function OmniAI() {
  ...
  const [selectedModel, setModel] = useState('assist');
      ...
      <header style={styles.header}>
        <div style={styles.headerLeft}>
          ...
          <div>
            ...
            <p style={styles.subtitle}>Multi-Model AI Orchestration · GIS & Housing Stack</p>
          </div>
        </div>
      ...
      <main style={styles.messages}>
        {messages.length === 0 && (
          <div style={styles.welcome}>
            ...
            <p>
              {selectedModel === 'assist'
                ? 'Assist mode routes each question to the right AI: Claude for code & docs, Perplexity for research, Gemini for maps & data, GPT-4o for planning & writing.'
                : 'Ask anything — I\'ll route it to the best AI for your question.'}
            </p>
            <div style={styles.suggestions}>
              {[
                '🗺️ Map zoning constraints for parcel [ID] in [city] — what\'s allowed under current code?',
                '💰 Draft a 9% LIHTC developer fee waterfall for a 100-unit project in [state].',
                '📰 Summarize the latest changes to LIHTC allocation rules in 2026 for [state].',
                '🐍 Write a Python script to batch-clip shapefiles to a study area boundary in QGIS.',
                '📄 Analyze this planning document and list the key zoning constraints.',
                '✍️ Help me write a professional email about [topic]',
              ].map(suggestion => (
                <button
                  key={suggestion}
                  style={styles.suggestionBtn}
                  onClick={() => setInput(suggestion.replace(/\[.*?\]/g, '').trim())}
                >
                  {suggestion}
                </button>
              ))}
        ...
        {messages.map(msg => (
          ...
          >
            <div style={styles.avatar}>
              {msg.role === 'user' ? '👤' : getModelEmoji(msg.routedTo || msg.aiModel)}
            </div>
            <div
              style={{
                ...styles.bubble,
            ...
            >
              {msg.role === 'assistant' && (
                <div style={styles.modelTag}>
                  {msg.aiModel === 'assist' && msg.routedTo
                    ? `Assist → ${ROUTED_TO_LABELS[msg.routedTo]?.emoji || ''} ${ROUTED_TO_LABELS[msg.routedTo]?.label || msg.routedTo}`
                    : getModelLabel(msg.routedTo || msg.aiModel)}
                </div>
              )}
      ...
      <footer style={styles.inputArea}>
        <div style={styles.inputRow}>
          <textarea
            ...
            placeholder={
              selectedModel === 'assist'
                ? 'Ask anything — Assist will route to the right AI (coding, research, maps, writing)…'
          ...
          <button
            ...
            style={{
              ...styles.sendBtn,
              backgroundColor: isLoading || !input.trim() ? '#9ca3af' : currentModel.color,
